...
for i = 1:3
    ...
    for ch = 1:numChannels(i)
        for f = 1:6
            ...
            %feats = [feats features{i, ch, f}(1:5998)];
        end
    end
    ...
    for finger = 1:5
        ...
        %normalized, N = 3: 30, 30, 50
        %not normalized, N = 3: 20, 20, 40
        %normalized, N = 5: 40, 40, 60 (with all PCA)
        %normalized, N = 7: 45, 45, 60 
        
        figure
        ncomp = 50;
        %[XL,YL,XS,YS,BETA,PCTVAR,MSE] = plsregress(pcaR,pos,ncomp) 
        ...
        %figure
        [XL,YL,XS,YS,BETA,PCTVAR,MSE] = plsregress(R,pos,ncomp) 
        %plot(1:50,cumsum(100*PCTVAR(2,:)),'-bo');
        %xlabel('Number of PLS components');
        %ylabel('Percent Variance Explained in y');
        
        est_pos = R*BETA(1:end-1)
        BETAS{i, finger} = BETA;
        
        %Nonlinear regression idea
...
for i = 1:3
    ...
    for finger = 1:5
        ...
        est_pos = R*BETAS{i, finger}(1:end-1)
        x = est_pos(1)*ones(N+1, 1);
        est_pos = [x; est_pos];  
        est_pos = est_pos(1:2950); %hack
